...
vector<string> WebSocket::explode(string theString, string theDelimiter,
	...
	while (end != string::npos) {
		...
		if (theIncludeEmptyStrings || ((length > 0) /* At end, end == length == string::npos */
...
int WebSocket::makeFrame(WebSocketFrameType frame_type, unsigned char* msg,
...
WebSocketFrameType WebSocket::getFrame(unsigned char* in_buffer, int in_length,
	...
	if (length_field <= 125) {
		...
		memcpy(&payload_length, &in_buffer[2], 2);
		payload_length = ntohs(payload_length);
		pos += 2;
	} else if (length_field == 127) { //msglen is 64bit!
		memcpy(&payload_length, &in_buffer[2], 4);
